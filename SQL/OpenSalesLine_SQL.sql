...
A.SALESID,
C.EnglishName,
(select sum(AvailPhysical) from inventSum sum1
...
where sum1.itemId=A.itemid and B.configId=b1.configId and  B.inventsiteId=b1.InventSiteid) as RTOnhand,--SUP_RTInventQtyOnHand
(select sum(AvailPhysical) from inventSum sum1
join inventDim  B1	on B1.dataareaid=sum1.dataareaid and B1.inventDimId=sum1.inventDimId 
